...
def run_rf(X_new, X_test_new, Y_train, Y_test,Y_name,groups):
...
#    grid_rf_debug={'n_estimators': list(range(100, 1100, 100)),'max_depth': list(range(2, 15))}
    # Randomized search:
    rf_regr_pipe_test = RandomizedSearchCV(estimator = model, param_distributions = grid_rf, scoring = scoring_regr, 
    ...
    # Save results
    best_results_rf = util.save_results_cv_pipe(rf_regr_pipe_test, model_name, "regr", scoring_regr,Y_name,X_new)
    print("Best results:")
    ...
    start = perf_counter()
    feat_imp_rf = util.calc_feature_importance(rf, X_test_new, Y_test, model_name, "regr",Y_name)
    stop = perf_counter()
    print("Time: ", timedelta(seconds = stop -start))
