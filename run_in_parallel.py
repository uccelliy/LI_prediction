...
def run_models_for_behavior(behav_name, X, Y_all,model_type):
    ...
    df_perf_init.to_csv(f'../results/performance_{model_type}_{behav_name}.csv')

    # 并行运行 RF/SVM/XGB
    Parallel(n_jobs=3)(
        delayed(model_func)(X_train, X_test, Y_train, Y_test, behav_name, groups,model_type)
        for model_func in [RF.run_rf, SVM.run_svm, XGB.run_xgb]
    )

    # Stacking 必须等待前三个模型完成
    Stack.run_stack(X_train, X_test, Y_train, Y_test, behav_name, groups,model_type)
...
def main():
    ...
    Parallel(n_jobs=os.cpu_count() - 1)(
        delayed(run_models_for_behavior)(behav_name, X, Y_all[["IID","FID",behav_name]],model_type)
